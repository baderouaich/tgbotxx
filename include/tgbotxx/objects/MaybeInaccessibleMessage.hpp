...
namespace tgbotxx {
  ...
  /// - Message
  /// - InaccessibleMessage
  /// @tip: To find which type is held, use:
  ///   - std::holds_alternative<T>(variant) with std::get<T>(variant)
  ///   - or test variant.index() against 0 or 1, where 0 = Ptr<Message> and 1 = Ptr<InaccessibleMessage>
